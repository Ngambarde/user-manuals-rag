    ...
    def test_gcs_config_default_bucket(self):
        ...
        with patch.dict(os.environ, {
            ...
            with patch.object(RAGSystem, '_initialize_components'):
                ...
                assert rag_system.config.use_gcs_vector_store is True
                assert rag_system.config.vector_store_bucket == "test-project-vector-stores"

    @pytest.mark.parametrize("use_gcs,expected_bucket", [
        ("true", "test-project-vector-stores"),
        ("false", None),
        ("1", "test-project-vector-stores"),
        ("yes", "test-project-vector-stores"),
    ])
    def test_gcs_config_various_boolean_values(self, use_gcs: str, expected_bucket: str):
...
class TestGCSVectorStoreLoading:
    ...
    def test_gcs_loading_success(self, mock_rag_system: MagicMock, test_client: TestClient):
    ...
    def test_gcs_loading_with_storage_mock(self):
        ...
        with patch.object(RAGSystem, '_initialize_components'):
            ...
            with patch('google.cloud.storage.Client') as mock_client_class:
                ...
                # Mock FAISS loading
                with patch('langchain_community.vectorstores.FAISS.load_local') as mock_faiss_load:
                    mock_db = MagicMock()
    ...
    def test_gcs_loading_error_handling(self, error_type: Exception, expected_error: str):
...
class TestBackwardCompatibility:
    ...
    def test_system_info_includes_gcs_config(self, mock_rag_system: MagicMock, test_client: TestClient):
...
class TestGCSIntegration:
    ...
    def test_gcs_config_in_system_info(self, mock_rag_system: MagicMock, test_client: TestClient):
    ...
    @pytest.mark.integration
    def test_gcs_health_check_includes_vector_store_info(self, mock_rag_system: MagicMock, test_client: TestClient):
